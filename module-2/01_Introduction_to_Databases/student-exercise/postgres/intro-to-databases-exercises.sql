...
Select name, governmentform AS form_of_government, headofstate AS head_of_state FROM country WHERE continent = 'Europe';

-- 4. The name, population, surface area, and average life expectancy of all countries in Asia (51 rows)

Select name, population, surfacearea AS surface_area, lifeexpectancy AS average_life_expectancy FROM country WHERE continent = 'Asia';

-- 5. The name, country code, and population of all cities with a population greater than 8 million people (10 rows)

Select name, countrycode AS country_code, population FROM city WHERE population > 8000000;

-- 6. The name, country code, and population of all cities with a population less than one thousand people (11 rows)

Select name, countrycode AS country_code, population FROM city WHERE population < 1000;

-- 7. The name, continent, and GNP of all countries with a GNP greater than one trillion dollars (6 rows)

Select name, continent, gnp AS GNP FROM country WHERE GNP > 1000000.00;
...
SELECT name, continent, population, gnp AS GNP, lifeexpectancy AS Average_life_expectancy FROM country WHERE lifeexpectancy > 80;

-- 9. The name and population of all cities in the USA with a population of greater than 1 million people (9 rows)

SELECT name, population FROM city WHERE countrycode = 'USA' AND population > 1000000.00;

-- 10. The name and population of all cities in China with a population of greater than 1 million people (35 rows)

SELECT name, population FROM city WHERE countrycode = 'CHN' AND population > 1000000.00;
...
SELECT name, continent, headofstate AS Head_of_state FROM country WHERE governmentform LIKE '%Monarchy%';

-- 13. The name of all cities in the USA with a population between 1 million and 2 million people (6 rows)

SELECT name FROM city WHERE population > 1000000.00 AND population < 2000000.00 AND countrycode = 'USA'; 

-- 14. The name and region of all countries in North or South America except for countries in the Caribbean (27 rows)

SELECT name, region FROM country WHERE region LIKE '%America%'; 
...
SELECT name, population, gnp AS GNP FROM country WHERE gnp > 1000000.00 AND population < 100000000;
...
SELECT name, lifeexpectancy AS Average_life_expectancy FROM country WHERE continent = 'Oceania';
...
SELECT name, lifeexpectancy AS Average_life_expectancy FROM country WHERE continent = 'Oceania' AND lifeexpectancy <> 0;
...
SELECT name, continent, gnp AS GNP, lifeexpectancy AS Average_life_expectancy FROM country WHERE lifeexpectancy >= 70 AND gnp > 10000.00 AND gnp < 100000.00;
...
SELECT 2020 - indepyear AS number_of_years_since_independence FROM country WHERE indepyear <> 0;
